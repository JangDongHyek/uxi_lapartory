...
// 상품정보를 가져온다.
   ...
   // jangdh
   $search_sql = "";
   $search_sql .= "and prdcode = '{$prdcode}'";

   $sql = "select * from wiz_product where 1 $search_sql";
   $result = mysql_query($sql);

   if ($row = mysql_fetch_array($result)) {
	   $product_skin = json_decode($row['skin'],true);
   }
   // jangdh //
...
<h3>상품등록<span>상품 상세정보를 설정합니다.</span></h3>
  ...
  <div id="addinfo" style=display:show>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
	  <td>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_basic">
		  <tr>
			...
			<td width="85%">
			  ...
			  <table border="1" cellspacing="5" cellpadding="0">
				  <tr>
				  	<td>진정 </td>
					<td><input name="prd_skin[]" type="text" value="<?=$product_skin['calm']?>" size="15" class="input"></td>
				  </tr>
				  <tr>
				  	<td>브라이트닝 </td>
					<td><input name="prd_skin[]" type="text" value="<?=$product_skin['brightening']?>" size="15" class="input"></td>
				  </tr>
				  <tr>
				  	<td>보습 </td>
					<td><input name="prd_skin[]" type="text" value="<?=$product_skin['moisturizing']?>" size="15" class="input"></td>
				  </tr>
				  <tr>
				  	<td>탄력 </td>
					<td><input name="prd_skin[]" type="text" value="<?=$product_skin['elasticity']?>" size="15" class="input"></td>
				  </tr>
			  </table>
